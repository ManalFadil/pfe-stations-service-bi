...
Objectif :
Ce script reproduit, avec Apache Spark, l'analyse du chiffre
d'affaires par station deja realisee en SQL sur l'entrepot de
donnees (cf. Chapitre 5, section 5.2 du memoire).

Positionnement (cf. Chapitre 4, section 4.4) :
Le volume actuel de donnees (~280 000 lignes) ne justifie pas
en soi une architecture Big Data. Ce script constitue une preuve
de concept demontrant la faisabilite d'un passage a un traitement
distribue (Spark) si le volume, la frequence ou la variete des
donnees venaient a augmenter significativement (ex: extension du
reseau, capteurs de jaugeage en continu).

Usage :
    spark-submit analyse_ca_spark.py --input ventes_sample.csv

Le fichier d'entree attendu reprend le schema de la table
FAIT_VENTES exportee depuis l'entrepot de donnees SQL Server
(colonnes : id_station, station_designation, code_article,
designation, date_facture, prix, qte_livrees).
================================================================
"""
